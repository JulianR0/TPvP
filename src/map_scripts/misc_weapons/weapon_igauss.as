/* 
* Custom version of HL Gauss for InstaGib Mode - Snk.EmA
*/
...
const int GAUSS_DEFAULT_GIVE = 100;
...
class weapon_igauss : ScriptBasePlayerWeaponEntity
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheModel( "models/snk_ema/im/v_gauss.mdl" );
		...
		g_Game.PrecacheGeneric( "sprites/misc_weapons/weapon_igauss.txt" );
	}
	...
	bool Deploy()
	{
		...
		return self.DefaultDeploy( self.GetV_Model( "models/snk_ema/im/v_gauss.mdl" ), self.GetP_Model( "models/hl/p_gauss.mdl" ), GAUSS_DRAW, "gauss" );
	}
	...
	void PrimaryAttack()
	{
		...
		{
			self.PlayEmptySound();
			self.m_flNextPrimaryAttack = WeaponTimeBase() + 1.2;
			return;
		}
		...
		m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType, iAmmo - 5 );
	...
	void SecondaryAttack()
	{
		...
		else
		{
			...
			if ( WeaponTimeBase() >= m_flNextAmmoBurn && m_flNextAmmoBurn != 0 )
			{
				int iAmmo = m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType );
			...
			if ( m_flStartCharge < g_Engine.time - 10 )
			{
				...
				m_pPlayer.TakeDamage( m_pPlayer.pev, m_pPlayer.pev, 500, DMG_SHOCK );
	...
	void StartFire()
	{
		...
		{
			flDamage = 250 * ( ( g_Engine.time - m_flStartCharge ) / GetFullChargeTime() );
		}
		...
		while ( flDamage > 10 && nMaxHits > 0 )
		{
			...
			if ( pEntity.ReflectGauss() )
			{
				...
				if ( Z_n < 0.5 ) // 60 degrees
				{
					...
					// explode a bit
					g_WeaponFuncs.RadiusDamage( tr.vecEndPos, self.pev, m_pPlayer.pev, flDamage * Z_n, flDamage * 0.15, CLASS_NONE, DMG_BLAST );
				...
				{	
					...
					// DISABLED - No more wallgausing -Giegue
					flDamage = 0;
					/*
...
string GetIGaussName()
{
	return "weapon_igauss";
}

void RegisterIGauss()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_igauss", GetIGaussName() );
	g_ItemRegistry.RegisterWeapon( GetIGaussName(), "misc_weapons", "uranium" );
}
